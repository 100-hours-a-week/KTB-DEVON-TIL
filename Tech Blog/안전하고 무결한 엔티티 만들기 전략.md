...
기존에 DB에 저장된 `0` 값들이 모두 `PENDING`으로 해석된다. 데이터가 깨지는 것이다.

STRING을 사용하면 DB에 "ACTIVE", "INACTIVE"로 저장되어 순서 변경에 안전하고, DB만 봐도 무슨 상태인지 읽힌다.
...
`LocalDateTime`은 타임존 정보가 없다. 서버가 여러 지역에 있거나, 운영 환경 설정에 따라 혼란이 생긴다. `Instant`는 UTC 기준 절대 시간이므로 어디서든 일관되다.

사용자에게 보여줄 때만 해당 지역의 `LocalDateTime`으로 변환하면 된다.

생성/수정 시간은 BaseEntity를 상속받아 자동 관리한다.
...
엔티티까지 내려온 데이터는 이미 검증된 데이터여야 한다. 엔티티 검증은 "만약을 위한 안전장치"다.

### 순수 예외를 사용하는 이유

엔티티 검증에서는 커스텀 예외가 아닌 **순수 예외**(IllegalArgumentException, IllegalStateException)를 사용한다.

이유는 복합적이다:

1. **책임 소재 명확화**: 이 예외가 발생했다는 것은 상위 레이어의 검증이 누락되었다는 의미다. **백엔드 개발자가 처리해야 할 버그**다.

2. **도메인의 순수성**: 도메인 레이어에 커스텀 예외를 두면, 예외 클래스가 늘어날수록 도메인이 복잡해진다.

3. **예외 처리 전략 구분**: 비즈니스 예외는 사용자에게 친절한 메시지를 반환하고, 검증 예외는 500 에러로 로깅하고 개발자가 수정한다.
